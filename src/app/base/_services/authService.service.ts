...
export class AuthService implements CanActivate {
    ...
    public atCurrentAuthData: AuthData;
    ...
    reset(reset: any): Observable<Response> {
        ...
        let observ = this.post(this.atOptions.resetPasswordPath + "/" + reset.token, body);
        observ.subscribe(res => {
            res.json()
        }, _error => {
            return Observable.throw(_error.json());
        });
        return observ;
    }

    // sending request for password reset
    forget(reset: any): Observable<Response> {
        const body = JSON.stringify(reset);
        let observ = this.post(this.getUserPath() + 'auth/forget-password', body);
        observ.subscribe(res => {
            res.json()
        }, _error => {
            return Observable.throw(_error.json());
